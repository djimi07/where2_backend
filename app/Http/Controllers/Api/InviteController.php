    ...
    public function My_friends_list(request $request)
    {
        ...
        if (@$request->searchText)
        {
            ...
            foreach (@$response['data'] as $key => $val)
            {
                ...
                // $is_friend = Friend::where(function ($q) use ($val,$user) {
                //     $q->Where(['userId'=>$user->userId,'friendId'=>$val->userId])
                //         ->orWhere(['friendId'=>$user->userId,'userId'=>$val->userId]);
                //     })->first();
                ...
                if ($_Friend == true)
                {
                    ...
                    if ($checked_user->count())
                    {
                        ...
                        foreach ($checked_user as $val)
                        {
                            ...
                            $bar_name = Bar_Restaurant::where(['restaurantId' => $val->restaurantId])->first();
