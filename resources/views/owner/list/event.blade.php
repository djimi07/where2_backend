...
function search_user(page = 1, filter_status = "") {
    ...
    var url = api_url + 'event/get_all_event';
    var obj = {};
    obj.search_text = $('#search_text').val();
    obj.filter_status = $('#filter_status').val();
    obj.restaurantId = '{{$restaruantId}}';
    obj.order_by = $('#order_by').val();
    if (filter_status == 'FILTER')
        next_page = 1;
    search_and_load_more(url, obj, page, 'search_result_box', 'no-data-box', 'load_more_btn', 'loadmoreIcon');
}
search_user(1, 'FILTER');

function search_and_load_more(url, searchData, page, search_box_id, no_box_id, load_btn_id, more_icon) {
    if (page == 1)
        page = next_page;
    next_page++;
    var limit = 10;
...
$(document).on('submit', '#add_event', function(e) {
    ...
    var url = api_url + 'event/add_update_event';
...
$(document).on('mouseover', '#datetimepicker1', function() {
    $('#start').datepicker({
        format: 'yyyy-mm-dd',
        autoclose: true,
        startDate: 'd',
...
$(document).on('mouseover', "#end", function() {
...
function edit(id) {
    $.ajax({
        url: api_url + 'event/edit_event/' + id,
        type: 'GET',
        dataType: 'JSON',
        success: function(response) {
            OpenZPanel('Edit');
            ...
            if (response.data.eventType)
                $('#selectevent').find('option[value=' + response.data.eventType + ']').attr("selected",
                    "selected");
            if (response.data.eventType == 1) {
                $(document).find('.event').hide();
                $(document).find('.drink_special').show();
            }
            if (response.data.eventType == 2) {
                $(document).find('.event').show();
                $(document).find('.drink_special').hide();
            }
            if (response.data.eventName) {
                $('#event').find('option[value=' + response.data.eventName + ']').attr("selected",
                    "selected");
            }
            if (response.data.offer) {
                $('#offer_type').find('option[value="'+response.data.offer+'"]').attr("selected", "selected");
            }
        }
...
function delete_event(id) {
    var url = api_url + 'event/delete_event';
    iziToast.question({
        timeout: 2000,
        close: false,
        overlay: true,
...
function update_status(id, status, box_id) {
    ...
    var url = api_url + 'event/update_status';
    if (status == 0)
        confirm_message = "Are you sure you want to unpublish this Event?";
    if (status == 1)
        confirm_message = "Are you sure you want to publish this Event?";
    update_single_status(url, postData, box_id, status, confirm_message);
}

function previous_view() {

    var dataArray = $("#add_event").serializeArray(),
        data = {};
    $(dataArray).each(function(i, field) {
        data[field.name] = field.value;
    });
    $(document).find("#image_modal").attr("src", data['image_url']);
    if (data['eventType'] == 1) {
        $(document).find('#offer_modal').html(data['offer']);
        $(document).find('#long_description').html(data['description']);
        if (data['bogo'] == 1) {
            $(document).find('#bogo_modal').html('Buy One Get One Free');
        }
    }
    if (data['eventType'] == 2) {
        $(document).find('#offer_modal').html(data['eventname']);
        $(document).find('#long_description').html(data['description']);
    }

}

function upload_event_image(file_id, filed_id, image_preview_id) {
    ...
    if ($.inArray(fileType, validImageTypes) < 0) {
        ...
        var url = api_url + 'event/upload_image';
        upload_single_image(file_id, url, image_preview_id, filed_id);
    }
